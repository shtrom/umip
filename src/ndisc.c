 ...
 * on success. Otherwise, -EINVAL is returned on error (including
...
/* Grab the L2 address of provided interface (ifindex) and
 * write the mapped address suitable for use in Source/Target
 * Link-Layer Address Option of various ND packets in 'addr'
 * 'addr' is expected to be at least MAP_L2ADDR_MAX_SIZE.
 *
 * The length (>0) of constructed address copied to addr is returned
 * on success.
 * -1 is returned on error,
 *  0 is return if the interface is expected not to have a L2 address
 *
 ...
 *       ...) specific mapping are required and documented in
 *       specific RFC documents.
 */
		  ...
		  const struct in6_addr *target, uint32_t flags)
{
	...
	/* XXX FIXME: Check if RFC 4861 allows sending NA w/o including
	 *            a source link-layer address option --arno */
	if (len > 0) {
